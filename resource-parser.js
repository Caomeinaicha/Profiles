...
☑︎ 支持 𝐕𝐦𝐞𝐬𝐬/𝐒𝐒(𝐑/𝐃)/𝐇𝐓𝐓𝐏(𝐒)/𝐓𝐫𝐨𝐣𝐚𝐧/𝐐𝐮𝐚𝐧𝐭𝐮𝐦𝐮𝐥𝐭(𝐗)/𝐒𝐮𝐫𝐠𝐞/𝐂𝐥𝐚𝐬𝐡/𝐒𝐡𝐚𝐝𝐨𝐰𝐫𝐨𝐜𝐤𝐞𝐭/𝐋𝐨𝐨𝐧 格式
☑︎ 提供说明 1⃣️ 中的可选个性化参数(筛选、重命名 等)
❷ 𝐫𝐞𝐰𝐫𝐢𝐭𝐞(重写) & 𝐟𝐢𝐥𝐭𝐞𝐫(分流) 的 转换 & 筛选  
☑︎ 用于禁用远程引用中某(几)项 𝐫𝐞𝐰𝐫𝐢𝐭𝐞/𝐡𝐨𝐬𝐭𝐧𝐚𝐦𝐞/𝐟𝐢𝐥𝐭𝐞𝐫
☑︎ 𝐒𝐮𝐫𝐠𝐞/𝐂𝐥𝐚𝐬𝐡 类型规则 𝐥𝐢𝐬𝐭 与 模块 𝐦𝐨𝐝𝐮𝐥𝐞 的解析使用
----------------------------------------------------------
0️⃣ ⟦原始链接⟧ 后加 "#" 使用, 不同参数用 "&" 连接: 
❖ https://mysub.com#emoji=1&tfo=1&in=香港+台湾
❖ 本地资源片段引用, 请将参数 "#𝗶𝗻=𝘅𝘅𝘅." 填入文件第 ① 行 ❖
❖ 支持中文, "操作" 以下特殊字符时请先替换
  ∎ "+"⇒"%2B", 空格⇒"%20", "@"⇒"%40", "&"⇒"%26", "."⇒"\."

1️⃣ ⟦𝐬𝐞𝐫𝐯𝐞𝐫 节点⟧ ➠ 参数说明:
⦿ info=1, 开启通知提示机场流量信息(如有提供);
...
⦿ rename 重命名, "旧名@新名", "前缀@", "@后缀", 用 "+" 连接多个参数;
  ...
  ❖ 示范: "rename=香港@𝐇𝐊+[𝐒𝐒]@+@[1𝐗]+流量.0\.2🐾"
  ❖ 默认 emoji 先生效, 如想调换顺序, 请用 𝐫𝐫𝐧𝐚𝐦𝐞 参数
...
⦿ delreg, 利用正则表达式来删除 "节点名" 中的字段(慎用)
⦿ replace 参数, 正则替换节点中内容, 可用于重命名/更改加密方式等
  ❖ replace=regex1@𝐬𝐭𝐫1+regex2@𝐬𝐭𝐫2
  ❖ replace=𝐫𝐞𝐠𝐞𝐱1@ 则等效于 𝐝𝐞𝐥𝐫𝐞𝐠 参数
⦿ sort=1/-1/x/指定规则, 分别按节点名 正/逆/随机/指定规则 排序
  ❖ 指定规则是正则表达式或简单关键词, 用"<" 或 ">" 连接
  ❖ sort=🇭🇰>🇸🇬>🇯🇵>🇺🇸 , 靠前排序
  ❖ sort=IEPL<IPLC<BGP , 靠后排序
⦿ del=1, 当有重名节点时, 用此参数删除重复节点(默认改名并保留)
⦿ ⟦进阶参数⟧: 𝐬𝐟𝐢𝐥𝐭𝐞𝐫/𝐬𝐫𝐞𝐧𝐚𝐦𝐞, 传入一段 base64 编码的脚本, 可用于更为复杂的[过滤/重命名] 需求

2⃣️ ⟦𝐫𝐞𝐰𝐫𝐢𝐭𝐞 重写⟧/⟦𝐟𝐢𝐥𝐭𝐞𝐫 分流⟧ ➠ 参数说明:
⦿ in, out, 根据关键词 保留/禁用 相关分流、重写规则;
⦿ inhn, outhn, “保留/删除”主机名(𝐡𝐨𝐬𝐭𝐧𝐚𝐦𝐞);
  ❖ 示范: 禁用 "淘宝比价" 及 "weibo" 的 js 同主机名
  𝐡𝐭𝐭𝐩𝐬://𝐦𝐲𝐥𝐢𝐬𝐭#𝐨𝐮𝐭=tb_price.js+wb_ad.js&outhn=weibo
⦿ regex, 正则筛选, 请自行折腾正则表达式;
  ❖ 可与 in(hn)/out(hn) 一起使用，in(hn)/out(hn) 会优先执行;
  ❖ 对 𝐡𝐨𝐬𝐭𝐧𝐚𝐦𝐞 & 𝐫𝐞𝐰𝐫𝐢𝐭𝐞/𝐟𝐢𝐥𝐭𝐞𝐫 同时生效(慎用)
⦿ policy 参数, 用于直接指定策略组，或为 𝐒𝐮𝐫𝐠𝐞 类型 𝐫𝐮𝐥𝐞-𝐬𝐞𝐭 生成策略组(默认"𝐒𝐡𝐚𝐧"策略组);
⦿ replace 参数, 正则替换 𝐟𝐢𝐥𝐭𝐞𝐫/𝐫𝐞𝐰𝐫𝐢𝐭𝐞 内容, regex@newregex;
  ❖ 将淘宝比价中脚本替换成 lite 版本, tiktok 中 JP 换成 KR
    ∎ replace=(price)(.*)@$1_lite$2+jp@kr 
⦿ dst=rewrite/filter，分别为将 𝐦𝐨𝐝𝐮𝐥𝐞&𝐫𝐮𝐥𝐞-𝐬𝐞𝐭 转换成 重写/分流;
  ❖默认将 𝐦𝐨𝐝𝐮𝐥𝐞 转换到重写, 𝐫𝐮𝐥𝐞-𝐬𝐞𝐭 转成分流
  ❖把 𝐫𝐮𝐥𝐞-𝐬𝐞𝐭 中 𝐮𝐫𝐥-𝐫𝐞𝐠𝐞𝐱 转成重写时, 必须要加 dst=rewrite;
  ❖把 𝐦𝐨𝐝𝐮𝐥𝐞 中的分流规则转换时, 必须要加 dst=filter
⦿ cdn=1, 将 github 脚本的地址转换成免翻墙cdn.jsdelivr.net

3⃣️ 其他参数
⦿ 通知参数 ntf=0/1, 用于 关闭/打开 资源解析器的提示通知
  ❖ 𝐫𝐞𝐰𝐫𝐢𝐭𝐞/𝐟𝐢𝐥𝐭𝐞𝐫 默认“开启”通知提示, 以防规则误删除
  ❖ 𝐬𝐞𝐫𝐯𝐞𝐫 资源解析则默认”关闭“通知提示
...
var Ppolicy = mark0 && para1.indexOf("policy=") != -1 ? decodeURIComponent(para1.split("policy=")[1].split("&")[0]) : "Shan";
...
function Parser() {
  try {
    ...
    $notify("解析出现错误", "请点击发送链接反馈", err, bug_link);
  }
  //$notify("","",total)
  $done({ content: total });
}
...
function ParseUnkown(cnt){
  try {
    cnt = JSON.parse(cnt)
    if(cnt) {
      $notify("链接返回内容并非有效订阅"+ "⟦" + subtag + "⟧","请自行检查原始链接，返回内容️",JSON.stringify(cnt), bug_link)
    }
...
function RegCheck(total, typen, regpara) {
	if(total.length == 0){ 
		$notify(typen + "  ➟ " + "⟦" + subtag + "⟧", "筛选正则: regex=" + regpara, "筛选后剩余项为 0️⃣ , 请检查正则参数及原始链接", nan_link)
...
function Type_Check(subs) {
    ...
    if (subs.indexOf(html) != -1 && link0.indexOf("github.com" == -1)) {
      $notify("该链接返回内容有误", "点通知跳转以确认链接是否失效", link0, nan_link);
      type = "web";
    } else if (typeU == "nodes") {
      type = "Subs-B64Encode"
...
function Rewrite_Filter(subs, Pin, Pout,Preg) {
    ...
    if (Nlist.length == 0) { $notify("重写引用  ➟ " + "⟦" + subtag + "⟧", "筛选参数: " + pfi + pfo, "筛选后剩余rewrite规则数为 0️⃣ 条, 请检查参数及原始链接", nan_link) }
...
function HostNamecheck(content, parain, paraout) {
    ...
    if (nname.length == 0) {
        $notify("重写引用  ➟ " + "⟦" + subtag + "⟧", "筛选参数: " + pfihn + pfohn, "主机名 hostname 中剩余 0️⃣ 项, 请检查参数及原始链接", nan_link)
    }
...
function Rule_Handle(subs, Pout, Pin) {
    ...
    if (Tout != "" && Tout != null) { // 有 out 参数时
        ...
        } else { $notify("分流引用  ➟ " + "⟦" + subtag + "⟧", "禁用: " + Tout, "未发现任何匹配项, 请检查参数或原始链接", nan_link) }
        if (Tin != "" && Tin != null) {  //有 in 跟 out 参数时
            if (nlist.length > 0) {
                ...
                $notify("分流引用  ➟ " + "⟦" + subtag + "⟧", "保留:" + Tin + ",禁用: " + Tout, "筛选后剩余规则数为 0️⃣ 条, 请检查参数及原始链接", nan_link)
            }
        } else {// if Tin (No Tin)
            if (nlist.length == 0) {
                $notify("分流引用  ➟ " + "⟦" + subtag + "⟧", "禁用: " + Tout, "筛选后剩余规则数为 0️⃣ 条, 请检查参数及原始链接", nan_link)
            }
        }
...
function Rule_Policy(content) { //增加、替换 policy
    ...
    if (RuleK1.some(RuleCheck1)) {
        if (cnt.length == 3 && cnt.indexOf("no-resolve") == -1) {
            ply0 = Ppolicy != "Shan" ? Ppolicy : cnt[2]
            nn = cnt[0] + ", " + cnt[1] + ", " + ply0
        } else if (cnt.length == 2) { //Surge rule-set
            ply0 = Ppolicy != "Shan" ? Ppolicy : "Shan"
            nn = cnt[0] + ", " + cnt[1] + ", " + ply0
...
function Filter(servers, Pin, Pout) {
    ...
    if (Pntf0 == 1 && Delist.length >= 1) {//通知部分
        ...
        $notify("⟦" + subtag + "⟧" + "筛选后节点数为0️⃣", "请自行检查原始链接以及筛选参数", link0, sub_link);
    }
    return Nlist
}
...
function Clash2QX(cnt) {
  ...
  for (i=0; i<nl; i++){
    try{
      ...
      $notify(`该节点解析错误, 暂时已忽略处理`,`可点击通知反馈至 bot`,node,bug_link )
    }
  }
